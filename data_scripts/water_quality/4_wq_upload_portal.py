'''
Step 4: This script gets the current dated SWAMP water quality data file (from the export folder) and uploads it to the open data portal. If there is no directory in the export folder with the current date, then the script will not be able to locate the correct file and the script will not run.
...
if __name__ == '__main__':
    ...
    print('--- Uploading water quality data file')
    ...
    substring = 'swamp_water_quality_data'
    ...
    # Upload file
    cu.upload_chunked_data(p_constants.portal_resource_ids['water_quality'], file_path, (1024 * 1024 * 64)) # 64MB chunk 
    ...
    # resource_info = ckan.action.resource_show(id=p_constants.portal_resource_ids['water_quality'])
